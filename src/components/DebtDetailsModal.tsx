    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:max-w-2xl max-h-[85vh] flex flex-col p-0 overflow-hidden gap-0">
        ...
        <Tabs defaultValue="overview" className="flex-1 flex flex-col overflow-hidden">
          <TabsList className="grid w-full grid-cols-3 mx-5 max-w-[calc(100%-2.5rem)] flex-shrink-0">
          ...
          {/* Overview Tab */}
          <TabsContent value="overview" className="flex-1 overflow-y-auto px-5 pb-5 space-y-3 mt-3">
            {/* Progress card */}
            <div className="rounded-xl border p-3 sm:p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[11px] sm:text-sm text-muted-foreground">Progression</span>
                <span className="text-[11px] sm:text-sm font-medium font-mono tabular-nums">{progress.toFixed(1)}%</span>
              </div>
              <Progress value={progress} className="h-2 sm:h-3 mb-3" />
              <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                {/* KPI tiles — sized down from the full .ft-kpi-value clamp so
                    three figures fit a phone-width modal (QuickPreview precedent). */}
                <div className="ft-kpi p-2.5 sm:p-3 gap-1">
                  <p className="ft-kpi-label truncate">Total</p>
                  <p className={cn("ft-kpi-value text-[13px] sm:text-base truncate", isPrivacyMode && "ft-priv")}>
                    {formatCurrency(debt.total_amount)}
                  </p>
                </div>
                <div className="ft-kpi p-2.5 sm:p-3 gap-1">
                  <p className="ft-kpi-label truncate">Payé</p>
                  <p className={cn("ft-kpi-value text-[13px] sm:text-base text-pos truncate", isPrivacyMode && "ft-priv")}>
                    {formatCurrency(amountPaid)}
                  </p>
                </div>
                <div className="ft-kpi p-2.5 sm:p-3 gap-1">
                  <p className="ft-kpi-label truncate">Restant</p>
                  <p className={cn("ft-kpi-value text-[13px] sm:text-base text-warn truncate", isPrivacyMode && "ft-priv")}>
            ...
            <div className="rounded-xl border p-3 sm:p-4 space-y-2.5">
              {debt.interest_rate > 0 && (
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[11px] sm:text-sm font-medium font-mono tabular-nums">{debt.interest_rate}%</span>
                </div>
              )}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[11px] sm:text-sm font-medium">
                    <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>
                      {formatCurrency(nextScheduled?.scheduled_amount ?? debt.payment_amount)}
                    </span>
                    {' / '}{getFrequencyLabel(debt.payment_frequency)?.toLowerCase()}
                  </span>
                </div>
              )}
              <div className="flex justify-between items-center">
            ...
            {nextScheduled && (
              <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 sm:p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={cn("text-sm sm:text-base font-semibold font-mono tabular-nums text-accent-deep", isPrivacyMode && "ft-priv")}>
          ...
          {/* Payments Tab */}
          <TabsContent value="payments" className="flex-1 overflow-y-auto px-5 pb-5 mt-3">
                ...
                <div className="flex flex-col gap-1 mb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs sm:text-sm font-medium">
                      Total:{' '}
                      <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>
                        {formatCurrency(debtPayments.reduce((s, p) => s + p.amount, 0))}
                      </span>
                    </p>
                  </div>
                  {debtPayments.some(p => p.principal_amount > 0) && (
                    <div className="flex items-center justify-end gap-2 text-[10px] sm:text-xs text-muted-foreground flex-wrap">
                      <span>Capital: <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>{formatCurrency(debtPayments.reduce((s, p) => s + (p.principal_amount || 0), 0))}</span></span>
                      <span>·</span>
                      <span>Intérêts: <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>{formatCurrency(debtPayments.reduce((s, p) => s + (p.interest_amount || 0), 0))}</span></span>
                      {debtPayments.some(p => (p.insurance_amount || 0) > 0) && (
                        <>
                          <span>·</span>
                          <span>Assurance: <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>{formatCurrency(debtPayments.reduce((s, p) => s + (p.insurance_amount || 0), 0))}</span></span>
                        </>
                      )}
                    </div>
                  )}
                  ...
                  <div key={payment.id} className="rounded-lg border p-2.5 sm:p-3 group">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-pos/10 flex items-center justify-center flex-shrink-0">
                          <CheckCircle2 className="w-4 h-4 sm:w-4.5 sm:h-4.5 text-pos" />
                        </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className={cn("text-xs sm:text-sm font-semibold font-mono tabular-nums text-pos whitespace-nowrap", isPrivacyMode && "ft-priv")}>
          ...
          {/* Schedule Tab */}
          <TabsContent value="schedule" className="flex-1 overflow-y-auto px-5 pb-5 mt-3">
            {loadingSchedule ? (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="ft-kpi p-2.5 sm:p-3 gap-1">
                    <p className="ft-kpi-label truncate">Payées</p>
                    <p className="ft-kpi-value text-base sm:text-lg text-pos">{paidScheduled.length}</p>
                  </div>
                  <div className="ft-kpi p-2.5 sm:p-3 gap-1">
                    <p className="ft-kpi-label truncate">Restantes</p>
                    <p className="ft-kpi-value text-base sm:text-lg text-warn">{unpaidScheduled.length}</p>
                  </div>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center">
                          {isPaid ? (
                            <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-pos" />
                          ) : isPast ? (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {(sp.principal_amount > 0 || sp.interest_amount > 0 || (sp.insurance_amount || 0) > 0) && (
                            <p className="text-[9px] sm:text-[10px] text-muted-foreground">
                              Capital: <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>{formatCurrency(sp.principal_amount)}</span>
                              {' · '}Intérêts: <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>{formatCurrency(sp.interest_amount)}</span>
                              {(sp.insurance_amount || 0) > 0 && (
                                <>
                                  {' · '}Assurance: <span className={cn("font-mono tabular-nums", isPrivacyMode && "ft-priv")}>{formatCurrency(sp.insurance_amount)}</span>
                                </>
                              )}
                            </p>
                          )}
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <span className={cn(
                            "text-[11px] sm:text-xs font-semibold font-mono tabular-nums whitespace-nowrap",
                            isPaid && "text-pos",
                            ...
                            <div className="flex items-center gap-0.5">
                              ...
                              >
                                {confirmingId === sp.id ? (
                                  ...
                                  <CheckCircle2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-pos" />
                                )}
                              </Button>
                              <Button
                                variant="ghost"
