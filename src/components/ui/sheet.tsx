...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card text-card-foreground border-line p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
...
interface SheetContentProps
  ...
  /** Suppress the built-in corner dismiss when the sheet's own header
   *  already provides one (it is absolutely positioned and would otherwise
   *  sit on top of the header's controls). */
  hideClose?: boolean;
}
